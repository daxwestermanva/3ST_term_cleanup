...
TEST:
	EXEC [Code].[UpdateMVIPersonSIDPatientPersonSID]

EXAMPLES:
	SELECT TOP 1000 * FROM [Common].[MVIPersonSIDPatientPersonSID]
	SELECT TOP 1000 * FROM [Common].[MVIPersonSIDPatientPersonSIDLog] ORDER BY UpdateDate DESC
...
DEPENDENCIES:
	Writes to:
		-- [Common].[Stage_MVIPersonSIDPatientPersonSID]
		-- [Common].[MVIPersonSIDPatientPersonSID]
		-- [Common].[MVIPersonSIDPatientPersonSIDLog]
	Reads from:
		-- [SPatient].[SPatient]
		-- [SVeteran].[SMVIPerson] 
...
*******************************************************************/
CREATE PROCEDURE [Code].[UpdateMVIPersonSIDPatientPersonSID]
AS
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @MergeOutput TABLE (
			...
			MERGE [Common].[MVIPersonSIDPatientPersonSID] WITH(TABLOCK) t USING #StagePatientPersonSIDToMVI s
			...
			INSERT INTO [Common].[MVIPersonSIDPatientPersonSIDLog] WITH(TABLOCK)
			SELECT
				PatientPersonSID	
				,Sta3n				
				,PatientICN			
