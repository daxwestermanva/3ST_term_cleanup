...
-- Modifications:
--	2024-09-06	LM	Add COLLATE SQL_Latin1_General_CP1_CS_AS to comparisons to retain changes to capitalization
-- =============================================
CREATE PROCEDURE [App].[VCL_CaringLettersWriteback]
...
IF --((LEN(@UpdateAddress1)=0 AND @UpdateAddress1<>' ') OR (LEN(@UpdateCity)=0 AND @UpdateCity<>' ') OR (LEN(@UpdateState)<>2 AND @UpdateState<>' ') OR (LEN(@UpdateZip)<>5 AND @UpdateZip <> ' ') OR (LEN(@UpdateCountry)=0 AND @UpdateCountry <>' ')) 
	...
	--OR (@PreferredName=1 AND (SELECT SUM(PreferredName) FROM [VCL].[CaringLettersMailings] WHERE MVIPersonSID=@MVIPersonSID)=0)
BEGIN;
	...
	--IF @PreferredName=1 AND (SELECT SUM(PreferredName) FROM [VCL].[CaringLettersMailings] WHERE MVIPersonSID=@MVIPersonSID)=0
...
SELECT a.MVIPersonSID
	...
	,a.DoNotSendReason
	,a.UsePreferredName
	,UpdateName = CASE WHEN (a.UpdateFirstName=b.FirstNameLegal COLLATE SQL_Latin1_General_CP1_CS_AS AND a.UpdateFullName=b.FullNameLegal)
		OR (a.UpdateFirstName=ISNULL(b.FirstNamePreferred COLLATE SQL_Latin1_General_CP1_CS_AS,'') AND a.UpdateFullName=ISNULL(b.FullNamePreferred,'')
...
--SELECT a.* 
--FROM #WritebackRecords a
--INNER JOIN [VCL].[CaringLettersWriteback] b
...
INSERT INTO [CaringLetters].[VCL_Writeback] 
SELECT * FROM #RecordChanges

UPDATE [CaringLetters].[VCL_Mailings]
SET ActiveRecord = 2
	,ActiveMailingRecord = 2
FROM [CaringLetters].[VCL_Mailings] a
...
SELECT a.MailingDate
	...
	,b.InsertDate
FROM (SELECT * FROM [CaringLetters].[VCL_Mailings] WHERE ActiveRecord=2) a
INNER JOIN #RecordChanges b ON a.MVIPersonSID = b.MVIPersonSID 
...
SET DoNotSend=0
	...
	,DoNotSendDate=NULL
FROM [CaringLetters].[VCL_Cohort] a
INNER JOIN #RecordChanges b ON a.MVIPersonSID = b.MVIPersonSID 
...
INNER JOIN #CheckPermissions c
	ON c.UserName=@User
LEFT JOIN [CaringLetters].[VCL_Writeback] AS wb WITH (NOLOCK)
