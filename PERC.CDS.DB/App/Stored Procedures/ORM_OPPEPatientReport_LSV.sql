...
-- Description: Drill down for OPPE report; code borrowed from App.ORM_PatientReport_LSV
...
-- 2022-07-14	CW	pointing to [PDW].[PDW_DWS_MasterPatient] instead of [App].[PDW_DWS_MasterPatient]
-- 2023-02-16   CW  changing "UDS" language to "Timely Drug Screen" (we're also including serum drug screens)
-- 2023-09-18   CW  Updating where clause re: [ORM].[OpioidHistory] - using ActiveRxStatusVM=1 to ensure Cerner data isn't cleaned out. 
--					Also adding inpatient census and facility.
-- 2024-06-17   TG  Removing the latest change and creating a separate Due in Ninety Days report
-- 2024-06-24   TG Fixing Drug Screen label in some cases (SUDdx_poss = 1)
-- 2024-12-09   TG Resolved unambiguous column issue
-- =============================================
CREATE PROCEDURE [App].[ORM_OPPEPatientReport_LSV]
...
SELECT DISTINCT mp.PatientICN, a.MVIPersonSID, o.ChronicOpioid
INTO  #cohort
FROM (  
	SELECT MVIPersonSID
		  ,Locations 
...
INSERT INTO #cohort
SELECT PatientICN,MVIPersonSID, 0 AS ChronicOpioid 
