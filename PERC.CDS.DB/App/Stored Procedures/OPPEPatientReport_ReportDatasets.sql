...
-- 20250718  - TG - Limiting the measures to the four in OPPE
-- =============================================
CREATE PROCEDURE [App].[OPPEPatientReport_ReportDatasets]
	@Report int  
	,@Dataset int  
...
BEGIN
 ...
 IF @Dataset = 1 -- GroupType Parameter 
	BEGIN
		...
		FROM [Present].[GroupAssignments_STORM] as a WITH (NOLOCK)
  ...
  ELSE IF @Dataset = 5 -- Cohort 
	BEGIN
            ...
            SELECT 'Long-Term Opioid Therapy' AS RowLabel, 8 AS RowValue
			      FROM [ORM].[PatientDetails] a WITH (NOLOCK)
				  INNER JOIN [ORM].[OpioidHistory] b WITH (NOLOCK)
				  ON a.MVIPersonSID = b.MVIPersonSID
				  WHERE b.ChronicOpioid = 1
				  ORDER BY RowLabel
