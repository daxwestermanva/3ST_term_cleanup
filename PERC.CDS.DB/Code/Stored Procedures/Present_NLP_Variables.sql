...
BEGIN
    ...
    WHERE Polarity = 'indicates_presence'
      AND (
          (Class = 'Psychological Pain' AND Subclass IN ('Pain exceeds tolerance', 'Housing issues', 'Sleep issues', 'Financial issues', 'Legal issues'))
    ...
    SELECT 
        ...
        CASE 
            WHEN a.TargetClass IN ('PPAIN', 'CAPACITY', 'JOBINSTABLE', 'JUSTICE', 'SLEEP', 'FOODINSECURE', 'DEBT', 'HOUSING') THEN '3ST' 
            ELSE NULL 
        END AS Category
    INTO #GetConcepts
    ...
    WHERE mvi.DateOfDeath_Combined IS NULL
      ...
      AND (
          (a.TargetClass IN ('PPAIN', 'CAPACITY') AND s.INSTANCE_ID IS NOT NULL)
          OR (a.TargetClass IN ('XYLA') AND (a.TargetSubClass = 'SUS' OR a.TargetSubClass = 'SUS-P'))
          OR a.TargetClass IN ('LIVESALONE', 'LONELINESS', 'DETOX', 'IDU', 'CAPACITY', 'JOBINSTABLE', 'JUSTICE', 'SLEEP', 'FOODINSECURE', 'DEBT', 'HOUSING')
      )
