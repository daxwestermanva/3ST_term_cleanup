...
MODIFICATIONS
	...
	2024-12-05	RAS	Changed NULL datetime to default '2100-12-31' to align with Oracle Health implementation (instead of 
					'9999-12-31') - trying to align this logic across system.
	2025-02-07	RAS	Removed join to MasterPatient what was being used in initial VistA query to create #main. I want to 
					run this before MasterPatient in the daily runs and the only purpose was to filter out test patients.
					It seems to make sense to just let this table be more expansive and downstream consumers can limit.
*/

CREATE PROCEDURE [Code].[Common_InpatientRecords_002]
AS
BEGIN
...
--ORDER BY 3
 ...
 EXEC [Maintenance].[PublishTable] 'Common.InpatientRecords_002','#StageIP'
